...
**爬取统计看板布局**（`/admin/crawler/stats`）：
```
┌─────────┬─────────┬─────────┬─────────┬─────────┬─────────┐
│ 总任务数 │ 总爬取数 │ 总成功数 │ 总失败数 │ 整体成功率│ 今日爬取 │  ← 爬取统计卡片
└─────────┴─────────┴─────────┴─────────┴─────────┴─────────┘
┌──────────────────────────┐  ┌──────────────────────────┐
│     近7日爬取趋势          │  │      数据源分布           │
│     (成功/失败折线图)       │  │      (饼图)              │
...
| 实现爬取统计页面 | `pages/Crawler/Stats.tsx` | Backend API |
| 实现统计卡片组件 | `components/CrawlerStatCard/` | - |
| 实现爬取趋势图 | `components/Chart/CrawlerTrendChart.tsx` | ECharts |
...
| 实现爬取统计页 | `pages/Crawler/Stats.tsx` | 爬取统计报表 |
| 响应式适配 | 全局 | 平板/手机端 |
| 性能优化 | 全局 | 懒加载、代码分割 |
