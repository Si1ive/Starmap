...
| 真实模拟考与练习 | 已完成（客观题主链） | `backend/app/modules/practice/router.py::create_practice_session`（L320-L388）从真实入库试卷冻结题面；`save_practice_answer`（L409-L456）按当前用户、服务器时限和乐观版本保存；`submit_practice_session`（L460-L468）确定性批改。`frontend/src/pages/PracticePage.tsx::PracticePage`（L39-L512）提供计时、作答、交卷、成绩与复盘 | 主观题目前只做标准答案精确匹配，未实现评分点辅助反馈；复杂公式/题图展示质量取决于现有抽取结果；不应宣称已完成可靠主观题自动评分 |
| 练习/模拟考区分、提示、番茄钟 | 已完成 | `backend/app/modules/practice/router.py::request_practice_hint`（L267-L316）只允许普通练习使用三级安全提示并记录证据，模拟考返回 409；同文件 `start_study_timer`、`complete_study_timer`（L561-L613）按用户持久化专注/休息实际时间；`frontend/src/pages/PracticeLibraryPage.tsx::PracticeLibraryPage`（L39-L309）提供两种开考入口和番茄钟 | 暂无用户自定义组卷筛选（章节/题型/难度）；提示是确定性学习脚手架，不是模型生成完整解题过程 |
...
3. 按薄弱关键词创建专项练习，并继续使用冻结题面、计时、版本冲突与确定性批改主链。
