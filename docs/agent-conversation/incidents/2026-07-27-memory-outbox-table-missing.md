...
| 启动期真表门禁 | `backend/app/modules/operations/schema_guard.py` | `verify_database_schema` | L44-L193 | Alembic revision、information_schema 表/列/索引 | 校验 Agent 必需表、Memory Outbox 复合唯一索引和模型列约束 | 校验通过才允许启动 Worker | 缺表/缺索引抛 `DatabaseSchemaError`，由 FastAPI lifespan 中止启动 |
| Worker 原始复现 | `backend/app/modules/agent/memory_outbox.py` | `MemoryOutboxStore._expire_exhausted_processing`、`MemoryOutboxStore.scan_due` | L61-L101 | 到期 processing/pending 任务 | 先把耗尽重试的任务置 failed，再查询可认领任务 | due 任务列表 | 数据库错误向 Worker 扫描循环传播 |
