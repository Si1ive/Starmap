...
| Worker 原始复现 | `backend/app/modules/agent/memory_outbox.py` | `MemoryOutboxStore._expire_exhausted_processing`、`MemoryOutboxStore.scan_due` | L56-L96 | 到期 processing/pending 任务 | 先把耗尽重试的任务置 failed，再查询可认领任务 | due 任务列表 | 数据库错误向 Worker 扫描循环传播 |
