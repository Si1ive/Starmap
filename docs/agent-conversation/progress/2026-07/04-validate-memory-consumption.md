...
- 实现：`turn_understanding.py::_derive_constraints`（L139-L152）抽取章节序号；`memory_selector.py::_resolve_explicit_chapter_ids`（L337-L388）解析并标记 explicit；`validate.py::_question_discovery_node`（L78-L159）阻止 unresolved 或下发 strict；`retrieve_knowledge`（L132-L345）与 `RetrievalService.search_with_outline_expansion`（L44-L120）保证大纲扩展不注入推测学科或额外章节。
