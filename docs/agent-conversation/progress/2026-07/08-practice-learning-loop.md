...
- 关键代码锚点：
  ...
  | 路由主链 | `backend/app/modules/agent/workflows/conversation.py` | `_route_node` | L60-L249 | 构建理解和 snapshot，注入 LearningSnapshot/只读 manifest，写 `conversation_decision`、兼容 `router_decision`、`teaching_policy_version` 与完整审计，决定 direct/clarify/child |
  ...
  | Child 交接 | `backend/app/modules/agent/workflows/conversation.py` | `_child_context_metadata`、`_dispatch_workflow_node` | L316-L367、L370-L411 | 将策略、flag snapshot 和上下文审计冻结到 child metadata，并保持原有 child Run/时间线幂等副作用 |
...
- Grade 与证据：`backend/app/modules/agent/workflows/grade.py::_open_answer_assessment_node`、`_open_answer_grading_evidence`（L210-L377）加入开放题分支，partial 按冻结 rubric 权重计算；`backend/app/modules/learning/evidence.py::EvidenceWeightPolicy.calculate`（L170-L249）增加答案可信度降权；`_record_grade_result_confirmed`（L315-L526）对 ungradable 或非 authoritative 灰度只记录活动、不创建掌握度事实。`record_agent_grade_activity`（L277-L440）保留 verdict、rubric、criterion/error tags、诊断回链和 flag policy version。
...
## 2026-07-29：阶段七前置——接入自适应学习版本灰度开关

- 目标：在不新增第二套任务队列或学习事实表的前提下，为 `conversation_decision_v2`、`learning_observer_v1`、`open_answer_assessor_v1` 和 `mastery_model_v2` 提供可独立关闭、shadow、canary 与 active 的服务端开关。
- 关键实现：新增 `backend/app/modules/agent/adaptive_learning_flags.py::AdaptiveLearningFeatureFlags.decision`（L111-L199）和 `snapshot`（L201-L214），使用 `sha256(flag:subject)%100` 稳定分桶，非法配置 fail closed；`Settings`（`backend/app/core/config.py` L174-L194）支持四项环境变量、canary 百分比和 `flag=mode[:percent]` 覆盖。
- 执行边界：`conversation._route_node`（L132-L224）把 shadow 决策留作审计但继续走兼容路径；`schedule_learning_observation`（`backend/app/modules/agent/learning_observer.py` L76-L124）只为入桶用户创建 silent Run；Grade 的 `_open_answer_assessment_node`（`backend/app/modules/agent/workflows/grade.py` L266-L377）和 `_record_grade_result_confirmed`（`backend/app/modules/agent/memory_projection.py` L315-L526）在关闭/shadow 时安全收敛，不产生未授权掌握度副作用。活动 payload 记录 flag policy version 和可用的 Run snapshot。
- 文档：同步更新 `implementation/routing-context-memory.md`、`implementation/events-timeline-errors.md`、`implementation/admin-observability.md`、任务路由和本任务状态；`pydantic-evals==2.14.1` 已加入后端依赖，评估数据集与指标仍在下一独立提交实现。
- 验证：`backend/venv/bin/python -m pytest tests/test_adaptive_learning_flags.py tests/test_agent_router_runtime.py tests/test_agent_learning_observer.py tests/test_agent_grade_worker.py tests/test_learning_evidence_model.py -q`（44 passed，9 个既有 datetime 弃用告警）；变更 Python 文件 Black/Flake8、`compileall` 和 `git diff --check` 通过。
- 中文提交信息：`增加自适应学习灰度开关`。

## 2026-07-29：阶段七评估内核——指标计算与权重校准候选

- 目标：为人工标注、Observer shadow 和固定场景复盘提供同一套脱敏指标口径，同时让权重校准只生成候选，不在评估运行中改动 `EvidenceWeightPolicy` 或线上掌握度语义。
- 关键实现：新增 `backend/app/modules/learning/adaptive_learning_metrics.py::AdaptiveLearningEvaluationSample`、`calculate_adaptive_learning_metrics`（L34-L238），固定主题解析、观察分类、评价一致、诊断触发、下一题预测、薄弱点恢复和工具越权七项指标；分母为零保留 `None`，工具策略违规按样本累计。
