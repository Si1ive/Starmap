...
- 在 `backend/app/modules/agent/memory_projection.py::_record_grade_result_confirmed`（L143-L247）校验 Feedback Artifact 的结构化评分证据，按用户 + evidence ID 写 `grade_result_confirmed`，并按知识点增量更新 `user_learning_mastery`；重复知识点先去重，重放同一证据无副作用。
