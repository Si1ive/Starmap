...
- 新增 `backend/tests/test_agent_plan_worker.py::test_rejected_plan_stops_without_outbox_or_artifact`（L89-L121）、`test_plan_apply_node_rejects_unapproved_checkpoint`（L125-L136）和 `test_approved_plan_resumes_and_creates_artifact`（L140-L166），同时锁定拒绝、旁路恢复和正常批准三条链路。
