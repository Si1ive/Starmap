...
| 5 | `backend/app/modules/agent/workflows/conversation.py` | `_child_context_metadata` | L219-L254 | 受控上下文、独立请求、snapshot ID、选中 capability 与父 Run `model_config_id` | 复制摘要 ID 等上下文审计、`active_topic`、`standalone_request`、`memory_snapshot_id`、模型配置与仅含当前能力的审计快照，不复制摘要正文、函数或 API Key | child run metadata | `_dispatch_workflow_node` |
| 6 | `backend/app/modules/agent/workflows/conversation.py`、`backend/app/modules/agent/capabilities.py` | `_dispatch_workflow_node`（L257-L307）、`CapabilityRegistry.require`（L56-L60） | Router action、parent/root run、独立请求 | action 必须命中能力目录和业务 workflow 白名单；随后幂等创建 compact child run，标题来自能力定义，`input_message` 使用 `standalone_request` | 队列中的 child run 与冻结 capability snapshot | `AgentWorker.process_run` |
