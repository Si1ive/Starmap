...
| 执行阶段 | 文件 | 符号 | 输入 | 处理 | 输出/副作用 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- |
| PracticeBundle 选择 | `backend/app/modules/agent/memory_selector.py` | `_resolve_explicit_chapter_ids`、`load_practice_bundle`、`_apply_explicit_question_repeat`（L534-L585、L638-L762、L1004-L1024） | child run、snapshot/items、知识点与章节约束、近期练习事实 | 校验权限；显式章节序号在唯一学科内解析并覆盖默认映射；排除集默认读取近期事实，只有 snapshot 同时含重复约束和唯一 question 引用时才从本轮视图移除该 ID，歧义时不放宽 | 含章节来源、解析状态和本轮排除视图的 `PracticeBundle`；事实事件不修改 | `_load_learning_evidence_node` |
