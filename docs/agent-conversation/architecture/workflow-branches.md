...
| 2 | `backend/app/modules/agent/turn_understanding.py`、`backend/app/modules/agent/memory_projection.py` | `_derive_constraints`、`build_turn_understanding`、`ensure_turn_memory_snapshot`、`project_topic_confirmed_fact` | L125-L193、L196-L281；L67-L122 | 当前输入、context refs、线程 `active_topic` | 生成确定性理解，抽取 difficulty 与章节序号，创建不可变 snapshot 并更新热状态；显式主题在 Router 前按 Run 幂等写事实，继承主题不重复写 | `standalone_request`、含章节约束的 snapshot、热状态、主题事实与 pending Outbox | `_route_node` 的 Router 调用 |
