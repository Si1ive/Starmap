...
| 讲解事实与学习活动投影 | `backend/app/modules/agent/memory_projection.py`、`backend/app/modules/learning/events.py`、`backend/app/modules/agent/memory_item_projection.py` | `project_completed_run_facts`、`_record_explanation_artifact_created`（L125-L185）；`record_explanation_activity`（L86-L130）；`project_trusted_memory_event`（L212-L224） | worker 已持久化的 explanation Artifact、冻结 active topic | 按 Run 幂等写讲解记忆事实与无 verdict 的学习活动，再确保 pending Outbox；活动不复制正文、不提高掌握度 | `explanation_artifact_created`、`agent_explanation_completed`、completed Outbox | 学习进度 / 后续摘要 projector |
...
| 评分事实、掌握度与薄弱点证据 | `backend/app/modules/agent/memory_projection.py`、`backend/app/modules/learning/events.py`、`backend/app/modules/agent/memory_item_projection.py` | `project_completed_run_facts`、`_record_grade_result_confirmed`（L308-L424）、`record_agent_grade_activity`（L145-L210）、`project_trusted_memory_event`（L212-L224） | Feedback Artifact 的 `content.grading`、冻结 topic | 校验证据、去重知识点，写幂等事实和掌握度，再写统一学习评价事件并确保 Outbox；缺主题/证据安全跳过 | 事实、掌握度、`agent_grade_confirmed` 与 completed Outbox | PlanningBundle / Weakness / 后续摘要 |
