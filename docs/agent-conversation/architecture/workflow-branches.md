...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `backend/app/modules/agent/workflows/conversation.py` | `_route_node` | L50-L155 | conversation run 与当前消息 | 构建近期原文与预算内 active 摘要；仅在真实歧义时调用指代模型，随后把选择冻结进 snapshot，并用独立请求及不可信摘要调用 Router | `agent_run_context`、RouterDecision 或失败 | `build_turn_understanding` / `ensure_turn_memory_snapshot` |
...
| --- | --- | --- | --- | --- | --- | --- |
| 对话连续性选择 | `backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/workflows/explain.py` | `load_conversation_bundle`（L829-L956）、`_load_scope_node`（L36-L47） | child run、同用户/线程 snapshot、冻结消息/Artifact ID | 只复现 snapshot 当时选中的 visible completed 消息和公开 Artifact；装载主题、引用与 deterministic query，删除旧的全学科固定 scope | `conversation_bundle`、`scope.mode=snapshot` | `_evidence_loop_node` |
| 有界资料探索 | `backend/app/modules/agent/workflows/explain.py` | `_conversation_inputs`、`_evidence_loop_node`（L50-L204） | ConversationBundle、预算和 child run ID | 把 bundle 转为受控 history/模型 deps；首次无论模型动作或 query 是什么，都强制使用唯一题面、topic aliases 或 standalone request 的冻结 query；后续只把成功非空结果记为 evidence并记录 decision/observation | 有效 evidence、`retrieval_attempted`、`retrieval_outcome` | `ExplanationRuntime.decide` / `retrieve_knowledge` |
...
| 结构化讲解生成 | `backend/app/modules/agent/workflows/explain.py` | `_fallback_evidence_text`、`_generate_explanation_node`（L234-L287） | standalone question、ConversationBundle history、证据列表和 `retrieval_outcome` | 截断证据正文并保留实体/来源；规划和正文模型复用同一份冻结 history、主题、Artifact 摘要与引用 ID；无资料时使用不同 fallback 并强制清空 citations | `ExplanationOutput` | `_citation_gate_node` |
...
| Artifact 渲染与结束 | `backend/app/modules/agent/workflows/explain.py` | `_render_artifact_node`、`_completed_node`（L305-L331） | outline、body、citations、summary | 组装 explanation artifact，并把最终 artifact 挂到 NodeResult 和上下文 | `agent_artifacts`、completed run | `AgentWorker.process_run` |
