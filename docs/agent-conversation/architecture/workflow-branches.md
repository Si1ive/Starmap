...
| 学习证据读取 | `backend/app/modules/agent/workflows/validate.py` | `_load_learning_evidence_node`（L51-L89） | `PracticeBundle`、Worker 注入的 FrozenTeachingPolicy | 先读取并冻结 `teaching_mode`、目标知识点和诊断标记，再用 bundle topic 填充 `weak_areas` / `recent_topics`；策略只写 ExecutionContext 和私有 Artifact metadata，不写学习事实 | `learning_evidence`、`practice_bundle`、teaching policy | `_question_discovery_node` |
...
| 练习草稿持久化 | `backend/app/modules/agent/workflows/validate.py`、`backend/app/modules/practice/service.py` | `_create_draft_node`（L290-L315）、`PracticeService.create_agent_draft`（L21-L125） | 有效题目、PracticeBundle、Run/User | 用 Run 做幂等键创建 draft Session；题库题重读完整 Question，模型题写 Session 原生快照且不进入公共题库；Run/所有权错误沿 WorkflowEngine 失败链传播 | `practice_sessions`、`practice_session_questions`、session ID | `_render_artifact_node` |
| Artifact 动作与结束 | `backend/app/modules/agent/workflows/validate.py`、`backend/app/modules/agent/worker.py` | `_render_artifact_node`（L318-L385）、`AgentWorker.process_run`（L185-L210） | Session ID、题面与组合信息 | 公开题面携带受控 `open_practice` 动作；模型答案/解析继续通过 `_private_metadata` 写入 Artifact 私有元数据 | 可跳转 Practice Artifact、私有批改元数据与 completed run | 用户练习页 / 后续 Grade |
...
| 作答快照 | `backend/app/modules/agent/workflows/grade.py` | `_load_attempt_snapshot_node`（L41-L86） | `EvaluationBundle`、Worker 注入的 FrozenTeachingPolicy | 先读取并校验 `feedback` 教学策略，再把题型、题面、标准答案、答案来源、知识点、来源 Artifact 与用户作答冻结进 ExecutionContext；bundle 不完整时返回失败 | `attempt` 与私有策略上下文；失败由 Worker 写 `run.failed`，无 Artifact/掌握度 | `_objective_grade_node` |
...
| 评分事实、掌握度与薄弱点证据 | `backend/app/modules/agent/memory_projection.py`、`backend/app/modules/learning/events.py`、`backend/app/modules/agent/mastery_projector.py`、`backend/app/modules/agent/memory_item_projection.py` | `project_completed_run_facts`（L129-L145）、`_record_grade_result_confirmed`（L315-L484）、`record_agent_grade_activity`（L241-L371）、`MasteryProjector.apply`（L35-L138）、`project_trusted_memory_event`（L212-L224） | Feedback Artifact 的 `content.grading`、冻结 topic、题目/知识点与答案来源 | 先按 `(user,evidence_id)` 幂等读取评分；`EvidenceGate` 校验来源 Run/User、题目、已验证知识点和可信答案，`EvidenceWeightPolicy` 按提示/来源/confidence/coverage 裁剪；再逐知识点调用 projector 更新 alpha/beta、兼容 score、uncertainty 和 evidence time，最后写统一活动事件并确保 Outbox；门禁失败不产生掌握度副作用 | `AgentMemoryEvent`、`UserLearningMastery`、带结构化 evidence 的 `agent_grade_confirmed` 与 completed Outbox | PlanningBundle / Weakness / 后续摘要 |
