...
| 15 | `backend/app/modules/agent/workflows/conversation.py` | `_direct_answer_node` | L104-L142 | `AgentRunContext` | 把 100ms 聚合后的正文 delta 写入事件并 commit；最终把完整输出包装成 message artifact | 可实时读取的 `message.delta` 与最终 assistant 内容 | `DirectAnswerRuntime.answer` |
...
| 执行序号 | 文件 | 符号 | 代码范围 | 输入 | 处理 | 输出/副作用 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `backend/app/modules/agent/workflows/conversation.py` | `_child_context_metadata` | L163-L186 | 受控上下文与父 Run 的 `model_config_id` | 只复制上下文审计和模型配置 ID，不复制 API Key | child metadata 固定本轮模型选择 | `_dispatch_workflow_node` |
| 2 | `backend/app/modules/agent/workflows/conversation.py` | `_dispatch_workflow_node` | L189-L234 | Router 选出的业务 action、父 Run 与受控上下文 | 幂等创建 compact child run，继承模型配置 ID，并创建对话内 workflow 时间线项 | 用户先看到真实排队中的业务卡片 | `AgentWorker.process_run` |
| 3 | `backend/app/modules/agent/worker.py` | `AgentWorker.process_run`（进入 running） | L127-L138 | queued child run | 写 `run.status_changed` 并立即 commit | SSE 可在节点执行前看到“执行中” | `WorkflowEngine.execute` |
| 4 | `backend/app/modules/agent/workflows/engine.py` | `WorkflowEngine.execute` | L61-L212 | workflow 定义、child Run 与执行上下文 | 每个节点开始写 `step.started` 并 commit；完成或失败后写对应事件再 commit；模型异常不再伪装为完成 | 真实步骤、错误和最终产物可恢复 | `build_explain_workflow` 注册的节点 |
| 5 | `backend/app/modules/agent/workflows/explain.py` | `build_explain_workflow` | L273-L297 | explain child Run | 定义 `load_scope → evidence_loop → evidence_gate → generate_explanation → citation_gate → render_artifact → completed` | 可执行节点图 | `_evidence_loop_node` |
| 6 | `backend/app/modules/agent/workflows/explain.py` | `_evidence_loop_node` | L40-L156 | 用户问题、模型调用预算和 child Run ID | 调用结构化资料规划；首次无论模型是否想结束都至少检索一次；仅把非空成功结果计为有效证据；模型异常直接返回失败 | `agent_loop_turns`、有效 evidence、`retrieval_attempted` 或失败结果 | `ExplanationRuntime.decide` / `retrieve_knowledge` |
| 7 | `backend/app/modules/agent/model_runtime/explanation.py` | `ExplanationRuntime.decide` / `ExplanationRuntime._run_decision` | L62-L101 / L145-L159 | 用户问题、有效资料数和 `ExplanationDeps.run_id` | 通过 `open_agent_model` 获取本轮模型，调用 Pydantic AI 并校验为 `LoopDecision`；只限制请求次数 | `retrieve_knowledge` / `finish` / `need_scope` 结构化动作 | `_evidence_loop_node` 执行动作 |
| 8 | `backend/app/modules/agent/model_runtime/config.py` | `open_agent_model` | L165-L217 | child Run ID | 从 child metadata 读取 `model_config_id`，解析 Agent 配置，创建带 API Key/Base URL 的独立客户端并回写审计元数据 | 与父会话一致的模型 session，退出时关闭客户端 | 资料规划或讲解正文模型调用 |
| 9 | `backend/app/modules/agent/tools/retrieve_knowledge.py` | `retrieve_knowledge` | L19-L180 | explain/validate 的查询词、范围和 run ID | 检索前写 `tool.called`；调用真实 RetrievalService；完成后写 `tool.result`；零命中公开“没有检索到相关文档”，异常公开“暂时无法检索相关文档” | 两次提交形成 running → completed/failed activity，不暴露内部术语 | `RetrievalService.search_with_outline_expansion` |
| 10 | `backend/app/modules/retrieval/service.py` | `RetrievalService.search_with_outline_expansion` | L44-L107 | 查询、学科/章节/实体过滤和 limit | 先用 canonical chapter 扩展查询，再执行 Qdrant dense/sparse hybrid 检索并合并过滤 | 真实命中结果与 outline expansion | `retrieve_knowledge` 精简公开结果 |
| 11 | `backend/app/modules/agent/workflows/explain.py` | `_evidence_gate_node` | L159-L175 | 有效 evidence 列表 | 非空才通过；空列表记录“没有检索到相关文档”，但允许模型基于可靠通用知识继续回答 | `gate_passed` 与公开原因 | `_generate_explanation_node` |
| 12 | `backend/app/modules/agent/workflows/explain.py` | `_generate_explanation_node` | L178-L225 | 用户问题和已筛选证据 | 截断并序列化资料；无资料时明确禁止伪造引用；调用结构化讲解运行时 | `ExplanationOutput` 写入执行上下文，错误向上返回 | `ExplanationRuntime.generate` / `_citation_gate_node` |
...
| 9 | `backend/app/modules/agent/model_runtime/router.py` | `RouterRuntime.decide` / `RouterRuntime._run` | L114-L180 / L182-L198 | 当前消息、筛选后的历史和运行配置 | 把条件生成的 `model_settings` 交给 Pydantic AI，校验 action；`UsageLimits` 只保留两次请求保护 | 供应商请求不出现伪无限输出值，也不受 4096 上下文选择预算误限 | 路由结果 |
