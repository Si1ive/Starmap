...
| 7 | `backend/app/modules/agent/model_runtime/explanation.py` | `ExplanationRuntime.decide` / `ExplanationRuntime._run_decision` | L62-L101 / L145-L162 | 用户问题、有效资料数和 `ExplanationDeps.run_id` | 通过 `open_agent_model` 获取本轮模型，调用 Pydantic AI 并校验为 `LoopDecision` | `retrieve_knowledge` / `finish` / `need_scope` 结构化动作 | `_evidence_loop_node` 执行动作 |
...
| 13 | `backend/app/modules/agent/model_runtime/explanation.py` | `ExplanationRuntime.generate` / `ExplanationRuntime._run_generation` | L103-L143 / L164-L181 | 问题、资料文本、child Run ID | 复用本轮 Agent 模型配置，调用 Pydantic AI 生成提纲、Markdown 正文、引用和总结 | 严格校验后的 `ExplanationOutput` | `_citation_gate_node` |
