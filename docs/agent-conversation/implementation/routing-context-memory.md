...
9. PlanningBundle 已接入 Plan，EvaluationBundle 已接入 Grade；硬编码学习证据和固定批改反馈均已移除。当前仍未实现 ConversationBundle、Explain 的 bundle 接入和历史摘要 bundle；结构化指代 unresolved 仍由 Router 决定是否澄清，不会静默猜测。
...
| Validate、Grade 与 Plan 已接入 bundle 化记忆 | `backend/app/modules/agent/memory_selector.py` `load_practice_bundle` / `load_evaluation_bundle` / `load_planning_bundle` | Validate 消费 `PracticeBundle`，Grade 消费 `EvaluationBundle`，Plan 消费 `PlanningBundle`；Explain / Router 仍缺 Conversation 类 bundle | `MEM-004`、`MEM-005` |
