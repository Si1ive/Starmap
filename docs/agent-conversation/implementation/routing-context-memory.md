...
| 完成事实投影 | `backend/app/modules/agent/memory_projection.py`（L19-L190） | `project_completed_run_facts`、`_record_practice_artifact_created`、`_record_grade_result_confirmed` | completed run、已持久化的 Artifact | 在 Run 完成事务内按 artifact 类型分派（不读 workflow 名）；practice 产物提取 `question_ids` 写幂等事实；feedback 只有携带结构化 `verdict`、`question_id`、`knowledge_point_ids` 时才按用户 + evidence ID 幂等写评分事实，知识点先去重，再按 verdict 增量更新掌握度 | `practice_artifact_created` / `grade_result_confirmed` 事实事件、`user_learning_mastery` 更新；证据缺失则无副作用返回；数据库异常沿 worker 完成事务传播 | `memory_selector._load_excluded_question_ids` / `_load_unique_weak_topic` |
...
| 掌握度投影已通但真实评分源未接入 | `backend/app/modules/agent/workflows/grade.py::_objective_grade_node`（L34-L51）、`_render_artifact_node`（L108-L137）；`backend/app/modules/agent/memory_projection.py::_record_grade_result_confirmed`（L86-L190） | 投影器只接受结构化真实评分证据并能安全更新掌握度；当前 P1 Grade 仍把所有作答视为主观固定反馈，不产生 verdict，所以不会误写但也没有线上掌握度证据 | `MEM-004`（EvaluationBundle）、`MEM-006`（真实评分生产者） |
| 长期回写尚未覆盖全部事实和 Outbox | `backend/app/modules/agent/worker.py::AgentWorker.process_run`（L183-L224）；`backend/app/modules/agent/memory_projection.py::project_completed_run_facts`（L19-L30） | Run 完成事务已分派 practice 与有真实证据的 feedback；主题确认、讲解 Artifact、计划确认以及 Memory Outbox 异步投影仍未实现 | `MEM-006` |
