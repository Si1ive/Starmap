...
| 独立请求与快照 | `backend/app/modules/agent/turn_understanding.py` | `build_turn_understanding`、`ensure_turn_memory_snapshot` | L92-L211 | 当前输入、context refs、线程 active topic | 确定性生成 `TurnUnderstanding`，保留 topic aliases，对“给我出道题”这类泛化输入补全 `standalone_request`，并创建 `agent_memory_snapshots` / `agent_memory_snapshot_items`、递增热状态版本 | `TurnUnderstanding`、`snapshot_id`、更新后的热状态 | `_route_node` |
...
| 首个 Bundle 选择器 | `backend/app/modules/agent/memory_selector.py` | `load_practice_bundle`、`build_practice_query` | L62-L150 | child run 的 `run_id` / `user_id`、snapshot metadata、selected snapshot items | 校验 run 与 snapshot 归属，从 snapshot 和选中的 `topic_focus` / `practice_generation` items 组装 `PracticeBundle`，并用 topic title + aliases 构造检索 query；无主题时返回空 query，避免静默默认主题 | `PracticeBundle`、确定性 query | `validate._load_learning_evidence_node` / `_question_discovery_node` |
...
3. `MEM-004` / `MEM-005` 的第一阶段已打通：Validate 会从 snapshot 装载 `PracticeBundle`，继承主题、别名、约束和选中的 Artifact，并据此生成检索 query。
4. 当前仍未实现歧义场景下的结构化指代消解模型，也还没有把掌握度、历史摘要和排除集真正做成可复用的 bundle。
...
| 只有 Validate 已接入 bundle 化记忆 | `backend/app/modules/agent/memory_selector.py` `load_practice_bundle` L62-L128；`backend/app/modules/agent/workflows/validate.py` `_load_learning_evidence_node` L39-L67 | Validate 已能消费 `PracticeBundle`；Explain / Grade / Plan 仍未声明并装载各自的 bundle | `MEM-004`、`MEM-005` |
| PracticeBundle 还未接入掌握度、排除集和澄清闭环 | `backend/app/modules/agent/memory_selector.py` `PracticeBundle` / `build_practice_query` L23-L31、L131-L150；`backend/app/modules/agent/workflows/validate.py` `_question_discovery_node` L70-L99 | 当前已移除静默默认主题，但还没有把 `user_learning_mastery`、历史排除题和“仍不唯一则澄清”做完 | `MEM-005`、`MEM-006` |
