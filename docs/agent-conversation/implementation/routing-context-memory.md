...
| LearningSnapshot 摘要 | `backend/app/modules/agent/learning_snapshot.py` | `LearningSnapshotSummary`、`_freeze_diagnostic_hypotheses`、`load_learning_snapshot_summary` | L30-L60、L128-L218、L221-L291 | 当前 Run 的 snapshot ID、同用户/线程、active topic、未过期 Observer 活动 | 读取 selected `learning_mastery` items；首次读取把 14 天内 diagnostic hypotheses 复制为 `learning_hypothesis` Snapshot items，后续只读冻结副本。稳定字段和知识点去重，过期/非法 payload 丢弃；找不到 snapshot 或非 UUID 兼容用户返回空 hypothesis | 同一 `learning_snapshot` 中的 mastery、diagnostic hypotheses、状态版本和 source item IDs；只新增当前 Run 冻结副本，不修改活动或 mastery | `RouterDeps.learning_snapshot` |
