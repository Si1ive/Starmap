...
| Conversation 路由、能力快照与显式主题事实 | `backend/app/modules/agent/workflows/conversation.py`、`backend/app/modules/agent/capabilities.py`、`backend/app/modules/agent/model_runtime/router.py` | `_route_node`（L60-L249）、`CapabilityRegistry.model_manifest` / `audit_manifest`（L62-L74）、`_router_policy`（L100-L122） | 完整 TurnUnderstanding、服务端能力目录、可选历史摘要和路由灰度决策 | snapshot 冻结当前理解和摘要副本；Router 只收到 action/key/description 的最小能力清单，并被明确告知不能直接写数据库、学习记录或薄弱点。选定能力、flag treatment 和完整安全属性写 root Run 审计快照；shadow/关闭按兼容决策继续 | `ConversationDecision`、可复现 memory snapshot、`capability_snapshot`、`adaptive_learning_flags` 与可选主题事实；异常交给 workflow engine | `_direct_answer_node` / `_dispatch_workflow_node` |
| Child 元数据交接 | `backend/app/modules/agent/workflows/conversation.py` | `_child_context_metadata` | L316-L367；父 run 的上下文审计、active topic、独立请求、选定能力、模型配置和 flag snapshot | 复制筛选后的消息/Artifact ID、`active_topic`、`standalone_request`、`memory_snapshot_id`、模型配置、当前能力审计和 `adaptive_learning_flags`，不复制敏感密钥或可执行函数 | child run metadata | `_dispatch_workflow_node` |
...
阶段七不再把“是否采用新版本”交给模型或客户端参数，而是由服务端按用户稳定
分桶决定，并把同一份决策写入 Run metadata。`shadow` 可以执行模型以收集评估
样本，但结果不会改变用户分支或权威掌握度；`canary` 和 `active` 才允许对应
版本进入既有副作用边界。四项开关可以独立关闭，避免一次回滚牵连 Observer、
开放题评分和掌握度投影。

| 执行阶段 | 文件 | 符号 | 代码范围 | 输入 | 处理 | 输出/副作用 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 开关配置 | `backend/app/core/config.py` | `Settings.ADAPTIVE_LEARNING_CONVERSATION_DECISION_V2`、`Settings.ADAPTIVE_LEARNING_LEARNING_OBSERVER_V1`、`Settings.ADAPTIVE_LEARNING_OPEN_ANSWER_ASSESSOR_V1`、`Settings.ADAPTIVE_LEARNING_MASTERY_MODEL_V2` | L174-L194 | 环境变量和 `ADAPTIVE_LEARNING_FLAG_OVERRIDES` | 解析 `disabled/shadow/canary/active` 及默认 canary 百分比；非法值由 resolver fail closed | 部署配置，不直接写业务事实 | `AdaptiveLearningFeatureFlags` |
| 稳定分桶 | `backend/app/modules/agent/adaptive_learning_flags.py` | `AdaptiveLearningFeatureFlags.decision`、`AdaptiveLearningFeatureFlags.snapshot` | L111-L214 | flag、用户 ID、Settings | 用 `sha256(flag:subject)%100` 得到稳定 bucket，区分 enabled、authoritative 和 shadow；生成四项版本审计快照 | `FeatureFlagDecision`；不访问数据库、不改变权限 | conversation/Observer/Grade/Mastery |
| 路由灰度 | `backend/app/modules/agent/workflows/conversation.py` | `_route_node` | L132-L224 | 已冻结 Understanding、LearningSnapshot 和路由 flag | active/canary 使用 Tutor；shadow 保存 `shadow_decision` 但用确定性兼容决策继续；关闭时显式任务仍走既有 action 护栏，普通输入回到 direct_answer | root Run 保存 flag/decision/policy metadata；shadow 不创建新 child 分支 | direct/clarify/dispatch |
| Observer 灰度 | `backend/app/modules/agent/learning_observer.py` | `schedule_learning_observation` | L76-L124 | 已完成根 conversation、用户 ID、Observer flag | 仅被分桶用户创建 silent child；Observer 本身仍只能写 `unknown + strength=0` 活动事实 | silent Run 带 flag snapshot；关闭时来源 Run 保持 completed 且不产生 Observer 副作用 | `learning_observation@v1` |
| 子 Run 版本传递 | `backend/app/modules/agent/workflows/conversation.py` | `_child_context_metadata` | L316-L367 | 父 Run 上下文、用户 ID、能力、教学策略和 flag snapshot | 将同一开关快照复制到 child metadata，防止 Grade/Explain/Validate 运行时重新读取漂移配置 | child Run 可回放本轮 rollout；公开 SSE 不展示内部字段 | child workflow |

开关错误只影响当前版本：路由关闭不会写学习证据，Observer 关闭不会回滚来源
conversation，Assessor 关闭或 shadow 会安全收敛为 `ungradable`，Mastery flag
关闭或 shadow 时仍可保留评分活动和审计，但跳过 `UserLearningMastery` 权威写入。
因此排障先检查 Run metadata 的 `adaptive_learning_flags` 和具体 rollout，再检查
EvidenceGate/ToolRegistry；不能仅从最终文案反推实际版本。
...
| 可信事实与 Outbox 生产 | `backend/app/modules/agent/memory_projection.py`（L27-L526） | `_ensure_memory_update_outbox`、`project_topic_confirmed_fact`、`project_completed_run_facts`、`_record_explanation_artifact_created`、`_record_plan_confirmed`、`_record_practice_artifact_created`、`_record_grade_result_confirmed` | snapshot 显式主题，或 completed run 的已持久化 Artifact | 写五类事实后在同一事务确保 pending Memory Outbox；Grade 在 L315-L526 先查证据幂等键，ungradable 只写活动，其余经过 EvidenceGate/WeightPolicy，并仅在 mastery flag authoritative 时交接 MasteryProjector；新事实先 flush 取得 ID，重放已有事实会补建缺失任务；SAVEPOINT 收敛数据库唯一键并发冲突，不污染外层 Run 事务 | `agent_memory_events`、`agent_memory_update_outbox`、`user_learning_mastery` 与 flag/policy 审计原子可见；不满足事实条件或灰度保护时不创建/更新掌握度 | 后续 Memory Outbox 消费者 / LearningSnapshot |
...
| 客观/开放题掌握度闭环 | `backend/app/modules/agent/workflows/grade.py::_load_attempt_snapshot_node`（L86-L145）、`_objective_grade_node`（L144-L207）、`_open_answer_assessment_node`（L266-L377）、`_render_artifact_node`（L516-L550）；`backend/app/modules/agent/memory_projection.py::_record_grade_result_confirmed`（L315-L526） | 选择、填空、判断题仍用可信标准答案确定性产生 verdict；开放题使用冻结 rubric 的 Assessor，partial 按服务端权重投影，低置信度/ungradable 或关闭/shadow 不写 mastery，全部结果都保留可回链 Artifact/活动事实 | `MEM-004`、`MEM-006` |
