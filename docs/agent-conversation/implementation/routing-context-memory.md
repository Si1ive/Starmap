...
| 异步派生仍缺 Embedding 与偏好候选 | `backend/app/modules/agent/conversation_summary.py::ConversationSummaryMaintainer.maintain`（L90-L211）；`backend/app/modules/agent/memory_item_projection.py::project_trusted_memory_event`（L154-L166） | Memory Outbox 已物化显式主题、批准计划并增量生成历史摘要；摘要已进入通用上下文和 Explain Bundle，但尚未向量化，偏好候选也未实现 | `MEM-006`、`MEM-007` |
