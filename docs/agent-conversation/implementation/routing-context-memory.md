...
10. 历史摘要已形成“异步生成→预算选择→snapshot 冻结→Router/普通回答消费”闭环；正文只作为动态 instructions 中的不可信数据，不伪装成 user history，也不进入公开 SSE。Child workflow 的专用 Bundle 和 Embedding 仍待实现。
...
| Explain、Validate、Grade 与 Plan 已接入 bundle 化记忆 | `backend/app/modules/agent/memory_selector.py` `load_conversation_bundle` / `load_practice_bundle` / `load_evaluation_bundle` / `load_planning_bundle` | 通用 Router/普通回答已消费冻结摘要；Explain/Validate/Grade/Plan 继续消费各自 Bundle，历史摘要尚未加入 child 专用 Bundle | `MEM-004`、`MEM-005`、`MEM-007` |
