...
| LearningSnapshot 摘要 | `backend/app/modules/agent/learning_snapshot.py` | `LearningSnapshotSummary`、`load_learning_snapshot_summary` | L23-L49、L75-L138 | 当前 Run 的 snapshot ID、同用户/线程、active topic | 只读取 selected 的 `learning_mastery` Snapshot Item，复制有限稳定字段并按知识点去重；找不到 snapshot 返回空摘要，不读取 live 掌握度，不产生写副作用；查询异常传播到 route | `learning_snapshot` 摘要、冻结状态版本和可校验知识点 ID | `RouterDeps.learning_snapshot` |
