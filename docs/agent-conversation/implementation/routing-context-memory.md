...
9. PlanningBundle 已接入 Plan，硬编码学习证据已移除；当前仍未实现 ConversationBundle、EvaluationBundle 和历史摘要 bundle。结构化指代 unresolved 仍由 Router 决定是否澄清，不会静默猜测。
...
| Validate 与 Plan 已接入 bundle 化记忆 | `backend/app/modules/agent/memory_selector.py` `load_practice_bundle` / `load_planning_bundle` | Validate 消费 `PracticeBundle`，Plan 消费 `PlanningBundle`；Explain / Grade 仍未声明并装载 Conversation/Evaluation 类 bundle | `MEM-004`、`MEM-005` |
...
| 异步派生仍缺摘要与 Embedding | `backend/app/modules/agent/memory_item_projection.py::project_trusted_memory_event`（L154-L166） | Memory Outbox 已运行，并物化显式主题与包含结构化 goals 的批准计划；Explain/Practice/Grade 只确认其既有权威落点。历史对话摘要、Embedding 和偏好候选仍未实现 | `MEM-006`、`MEM-007` |
