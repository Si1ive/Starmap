...
| Memory Outbox 消费内核 | `backend/app/modules/agent/memory_outbox.py`（L34-L285） | `MemoryOutboxStore.scan_due`、`MemoryOutboxStore.claim`、`MemoryOutboxStore.complete`、`MemoryOutboxStore.fail`、`MemoryOutboxConsumer.process_claimed`、`MemoryOutboxConsumer.scan_and_process` | pending 或租约过期的 processing 任务、worker ID、重试与租约参数 | 扫描到期任务，以条件 UPDATE 原子认领；`scheduled_at` 在 processing 状态表示租约截止；接管崩溃任务时累计重试，耗尽预算收敛 failed；投影前逐项校验 Outbox 与事实的 run/thread/user/type 归属，并在 SAVEPOINT 内调用 projector | 成功写 completed；异常只回滚派生投影并延迟重试/最终 failed，原 completed Run 不变；完成/失败均校验 worker 所有权 | 实际派生 projector 与 Agent Worker 运行时接入（待实现） |
...
| Memory Outbox 尚未接入实际派生投影 | `backend/app/modules/agent/memory_outbox.py::acknowledge_memory_event`（L25-L31）、`MemoryOutboxConsumer.scan_and_process`（L255-L285） | 消费状态机已支持扫描、原子认领、租约回收、所有权、重试和失败隔离；默认 projector 当前只校验可信事实类型，且尚未接入 Agent Worker，避免在摘要/Embedding/目标 projector 落地前过早完成任务 | `MEM-006` |
