...
| 本会话练习列表刷新 | `frontend/src/pages/AgentPage.tsx` | `AgentPage`（L117-L125） | 已打开的 `threadId` 或时间线 `latestCursor` 变化 | 调用 `listThreadPractices` 读取当前 Thread 的练习；请求失败收敛为空列表，不阻断对话时间线；返回值写入 `threadPractices` | `ConversationPracticeRail` 的 `items` |
| 本会话练习轨道渲染 | `frontend/src/features/agent/ConversationPracticeRail.tsx` | `ConversationPracticeRail`（L11-L55） | `items` 为空或包含 draft/active/submitted 练习 | 空列表时给侧栏设置 `aria-hidden` 和 `agent-practice-rail--empty`，不渲染标题/空态正文；有数据时按状态渲染轨道、题数和得分，并把按钮导航到练习页或反馈页；只产生浏览器路由变化，不写数据库 | 练习页或反馈页 |
| 本会话练习布局 | `frontend/src/pages/AgentPage.tsx`、`frontend/src/features/agent/agent-chat.css` | `AgentPage`（L275-L320）；`.agent-chat-page`、`.agent-practice-rail`、`.agent-practice-rail--empty`、`.agent-chat-rail-spacer`（L1-L20、L86-L111）；`@media (max-width: 900px)`（L1283-L1306） | 有会话页面渲染，桌面或窄屏断点 | 侧栏使用统一的 208px 宽度；对话流后的 spacer 让底部输入 dock 与侧栏边界对齐；空侧栏透明且不可交互，窄屏隐藏 spacer 并把空侧栏压到 0 高度，避免空练习区占用首屏 | 对话时间线、底部 `ChatComposer` 与响应式页面布局 |
| 最近学习记录 | `frontend/src/pages/TodayPage.tsx`、`frontend/src/api/learning.ts` | `TodayPage`（L260-L293）、`LearningActivity` / `LearningProgress`（L19-L44） | `recent_activities`、关键词轨迹来源 | Agent 讲解显示为活动，正确/错误练习显示评价结果；Session 回练习结果、Thread 回原对话；不把 exposure 文案写成掌握，页面不再消费学习时长字段 | 学习进度页 |
