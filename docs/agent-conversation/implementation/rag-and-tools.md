...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Explain/Validate 发起检索 | `backend/app/modules/agent/workflows/explain.py`、`backend/app/modules/agent/memory_selector.py`、`validate.py` | `_evidence_loop_node`、`build_practice_query`、`build_practice_filters`、`_question_discovery_node` | `explain.py` L49-L176；`memory_selector.py` L161-L193；`validate.py` L74-L106 | query 或 `PracticeBundle`、范围、run ID、可选 `entity_type` | 由 workflow 统一调用 `retrieve_knowledge`；Explain 使用混合知识检索并记录 `retrieval_outcome`；Validate 先按 `PracticeBundle` 中的 topic title + aliases 生成 query，再把 `knowledge_point_ids`、`difficulty` 和排除题 ID 作为过滤条件传给检索；若没有主题也没有 fallback terms，则直接失败，不再静默默认“数据结构/操作系统” | Tool 调用与内部结果，或缺题目主题错误 | `retrieve_knowledge` / workflow failure |
...
| `MEM-005` | `backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/tools/retrieve_knowledge.py`、`backend/app/modules/agent/workflows/validate.py`、`backend/app/modules/retrieval/search_engine.py` | `load_practice_bundle`、`build_practice_filters`、`retrieve_knowledge`、`_question_discovery_node`、`build_filter` | `memory_selector.py` L81-L193；`retrieve_knowledge.py` L130-L337；`validate.py` L43-L106；`search_engine.py` L104-L169、L449-L530 | 进行中：Validate 已开始消费 `PracticeBundle`，会用 snapshot topic aliases 构造 `query="二分查找 折半查找"`，并把 `knowledge_point_ids`、`difficulty` 和排除题 ID 下发到检索层；掌握度、真实排除集和澄清闭环仍待补齐 |
