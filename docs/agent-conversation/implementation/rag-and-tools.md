...
| --- | --- | --- | --- | --- | --- | --- |
| Explain/Validate 发起检索 | `backend/app/modules/agent/workflows/explain.py`、`backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/workflows/validate.py`、`backend/app/modules/agent/tools/retrieve_knowledge.py`、`backend/app/modules/retrieval/service.py` | `load_conversation_bundle`（L832-L1001）、`_evidence_loop_node`（L69-L206）、`_resolve_explicit_chapter_ids`、`_question_discovery_node`、`retrieve_knowledge`（L132-L345）、`RetrievalService.search_with_outline_expansion`（L44-L120） | ConversationBundle / PracticeBundle、范围、run ID、可选实体类型 | Explain 复现冻结 history/摘要，但首次查询仍固定使用唯一题面、topic aliases 或 standalone request；Validate 透传知识点、章节、难度和排除题 | Tool 调用、等待输入、安全失败，或冻结/严格范围内结果 | `retrieve_knowledge` / waiting checkpoint |
