...
| --- | --- | --- | --- | --- | --- | --- |
| Explain/Validate 发起检索 | `backend/app/modules/agent/workflows/explain.py`、`backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/workflows/validate.py`、`backend/app/modules/agent/tools/retrieve_knowledge.py`、`backend/app/modules/retrieval/service.py` | `load_conversation_bundle`（L829-L956）、`_evidence_loop_node`（L68-L204）、`_resolve_explicit_chapter_ids`、`_question_discovery_node`、`retrieve_knowledge`（L132-L345）、`RetrievalService.search_with_outline_expansion`（L44-L120） | ConversationBundle / PracticeBundle、范围、run ID、可选实体类型 | Explain 首次查询固定使用唯一 question 题面、topic title+aliases 或 standalone request，不能被模型首个 query 覆盖；Validate 透传知识点、章节、难度和排除题，显式章节使用 strict 且底层不注入推测范围 | Tool 调用、等待输入、安全失败，或冻结/严格范围内结果 | `retrieve_knowledge` / waiting checkpoint |
...
| `MEM-005` | `backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/tools/retrieve_knowledge.py`、`backend/app/modules/agent/workflows/validate.py`、`backend/app/modules/retrieval/search_engine.py`、`backend/app/modules/agent/service.py` | `load_practice_bundle`、`_resolve_explicit_chapter_ids`、`build_practice_filters`、`retrieve_knowledge`、`_question_discovery_node`、`build_filter`、`create_input`、`submit_input_answer` | 已完成首个闭环：Validate 消费 snapshot topic/aliases、知识点、显式或映射章节、难度、真实排除集与唯一薄弱点；缺主题可等待补充，无法解析显式章节则安全失败，不使用默认主题或宽检索 |
