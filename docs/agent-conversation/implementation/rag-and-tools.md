...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Explain/Validate 发起检索 | `backend/app/modules/agent/workflows/explain.py`、`backend/app/modules/agent/memory_selector.py`、`validate.py` | `_evidence_loop_node`、`build_practice_query`、`_question_discovery_node` | `explain.py` L49-L176；`memory_selector.py` L131-L150；`validate.py` L70-L99 | query 或 `PracticeBundle`、范围、run ID、可选 `entity_type` | 由 workflow 统一调用 `retrieve_knowledge`；Explain 使用混合知识检索并记录 `retrieval_outcome`；Validate 先按 `PracticeBundle` 中的 topic title + aliases 生成 query，再强制 `entity_type="question"`；若没有主题也没有 fallback terms，则直接失败，不再静默默认“数据结构/操作系统” | Tool 调用与内部结果，或缺题目主题错误 | `retrieve_knowledge` / workflow failure |
...
| `MEM-005` | `backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/workflows/validate.py` | `load_practice_bundle`、`build_practice_query`、`_load_learning_evidence_node`、`_question_discovery_node` | `memory_selector.py` L62-L150；`validate.py` L39-L99 | 进行中：Validate 已开始消费 `PracticeBundle`，会用 snapshot topic aliases 构造 `query="二分查找 折半查找"`，且无主题时直接失败，不再静默默认随机出题；掌握度、排除集和澄清闭环仍待补齐 |
...
| Validate 题目资格门读取真实 DTO 元数据，二分查找题能经检索 DTO 进入候选集，并优先使用 snapshot topic aliases 构造 query | `backend/tests/test_agent_validate_workflow.py` | `test_question_gate_accepts_rich_question_metadata_without_source_type`、`test_question_gate_filters_deleted_or_source_less_questions`、`test_validate_binary_search_question_survives_retrieval_dto_and_gate`、`test_validate_uses_practice_bundle_topic_for_query`、`test_validate_stops_when_no_topic_or_fallback_terms` | L18-L237 |
| `PracticeBundle` 会从 snapshot、selected items 和 selection metadata 组装主题、约束与 Artifact 选择 | `backend/tests/test_agent_memory_selector.py` | `test_load_practice_bundle_uses_snapshot_topic_and_context_metadata` | L46-L130 |
