...
| --- | --- | --- | --- | --- | --- | --- |
| Explain/Validate 发起检索 | `backend/app/modules/agent/workflows/explain.py`、`backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/workflows/validate.py`、`backend/app/modules/agent/tools/retrieve_knowledge.py`、`backend/app/modules/retrieval/service.py` | `_evidence_loop_node`、`_resolve_explicit_chapter_ids`、`_question_discovery_node`、`retrieve_knowledge`（L132-L345）、`RetrievalService.search_with_outline_expansion`（L44-L120） | query 或 `PracticeBundle`、范围、run ID、可选实体类型 | Validate 透传知识点、章节、难度和排除题；显式章节设置 `strict_chapter_scope`，工具将其纳入活动幂等键与公开元数据，底层大纲扩展仍增强 dense query，但不注入推测学科或额外章节；无法解析时工具调用前失败。知识点映射章节保持原扩展语义 | Tool 调用、等待输入、安全失败，或严格范围内结果 | `retrieve_knowledge` / waiting checkpoint |
