...
| --- | --- | --- | --- | --- | --- | --- |
| 冻结检索焦点 | `backend/app/modules/agent/turn_understanding.py` | `TurnUnderstanding`（L65-L73）、`_derive_retrieval_query`（L127-L158）、`build_turn_understanding`（L382-L442） | 当前用户原文、可信 `context_refs` 或线程 `active_topic` | 已有可信主题时组合标题与别名；新主题只剥离“给我讲解一下/请详细解释/帮我出题”等交互外壳，保留“三次握手为什么”等问题焦点，限制 160 字。抽取结果是本轮 `retrieval_query`，不会凭空创建带实体 ID 的长期主题 | `AgentMemorySnapshot.understanding_json.retrieval_query`；纯字符串处理，无模型、向量或外部副作用 | `load_conversation_bundle` |
...
| Agent 结果归一化 | `backend/app/modules/agent/tools/retrieve_knowledge.py` | `_decode_text`（L25-L34）、`_agent_result_title`（L37-L45）、`_normalize_agent_result`（L48-L72） | `RetrievalResult.to_dict()` 输出 | 对历史索引中残留的字面量 `\\uXXXX` 做局部 Unicode 反转义，同时保留正常中文；再统一成 Agent 可消费的实体、正文、上下文、章节和来源 DTO | 可读中文 Agent DTO；无数据库副作用 | Explain/Validate 工作流 |
...
| Unicode 编码归一化与知识库来源区块 | `backend/tests/test_agent_rag_presentation.py` | `test_retrieval_result_decodes_literal_unicode_escapes`、`test_explanation_artifact_precedes_body_with_knowledge_sources` |
