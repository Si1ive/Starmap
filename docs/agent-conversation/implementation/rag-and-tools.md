...
| --- | --- | --- | --- | --- | --- | --- |
| Explain/Validate 发起检索 | `backend/app/modules/agent/workflows/explain.py`、`backend/app/modules/agent/memory_selector.py`、`validate.py` | `_evidence_loop_node`、`build_practice_query`、`build_practice_filters`、`_question_discovery_node` | query 或 `PracticeBundle`、范围、run ID、可选 `entity_type` | 由 workflow 统一调用 `retrieve_knowledge`；Explain 使用混合知识检索并记录 `retrieval_outcome`；Validate 先按 `PracticeBundle` 中的 topic title + aliases 生成 query，再把 `knowledge_point_ids`、`difficulty` 和排除题 ID 作为过滤条件传给检索；若没有主题且没有 fallback terms，则先读取或创建 `practice_topic` 输入项，等待用户补充后从断点恢复并继续检索，而不是静默默认主题或直接失败 | Tool 调用、等待用户输入，或内部检索结果 | `retrieve_knowledge` / waiting checkpoint |
