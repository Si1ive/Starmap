...
| 固定场景 Evals | `backend/evals/adaptive_learning.py` | `AdaptiveLearningMetricsEvaluator.evaluate`、`fixed_adaptive_learning_cases`、`build_adaptive_learning_evaluation_dataset`、`run_fixed_adaptive_learning_evals_sync` | L53-L80、L145-L287 | 十个脱敏 Case，覆盖问询、无作答、客观题、提示、变式题、开放题、重试和工具拒绝 | Pydantic Evals 运行无副作用 fixture task，调用同一指标函数；报告按 evaluator version 聚合，工具门禁和 replay key 另做安全断言 | `EvaluationReport`；不调用模型、不读写数据库，结果可供 CI/发布前人工比较 | 查看 `report.averages()` 和 evaluator version |
