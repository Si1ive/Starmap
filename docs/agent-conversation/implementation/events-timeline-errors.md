...
| 执行阶段 | 文件 | 符号 | 代码范围 | 输入 | 处理 | 输出/副作用 | 下一步 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 评分证据构造 | `backend/app/modules/learning/evidence.py` | `build_assessment_evidence` | L252-L333 | 服务端 grading、题目 ID、知识点、答案来源、提示/答案暴露、评分/答案 confidence | 去重知识点并补均分 coverage；把 verdict、答案来源和已知错误标签归一化为唯一 Pydantic 契约；忽略未知技术标签，不接受 mastery 字段 | `LearningEvidence`，尚未写库 | `EvidenceGate.validate` |
...
| 离线评估与权重候选 | `backend/app/modules/learning/adaptive_learning_metrics.py`、`backend/app/modules/learning/evidence.py` | `calculate_adaptive_learning_metrics`、`calibrate_weight_caps`、`EvidenceWeightPolicy.source_caps` | metrics L34-L238、L241-L374；evidence L164-L168 | 结构化脱敏样本、当前 policy cap、人工参考结果 | 分母不足时指标为 `None`；校准只按来源汇总 agreement/MAE，并在达到样本门槛时生成不超过当前 cap 的候选 | 指标报告和待批准校准报告；不产生 LearningActivityEvent、mastery 或线上策略变更 | 交给固定 Evals 与人工审批流程 |
| 固定场景回放 | `backend/evals/adaptive_learning.py` | `AdaptiveLearningMetricsEvaluator.evaluate`、`run_fixed_adaptive_learning_evals_sync` | L53-L80、L278-L287 | 固定十场景 Dataset、版本化 evaluator | Pydantic Evals 只回显脱敏 fixture，再聚合七项指标、工具安全和重试键断言；没有模型、数据库或事件副作用 | `EvaluationReport`；`report.averages()` 消费聚合结果，历史报告由 evaluator version 区分 | 发布前比较版本，异常时回到 flag/权重报告 |
