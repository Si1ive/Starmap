...
| 可信事实投影 | `backend/app/modules/agent/memory_projection.py` | `project_topic_confirmed_fact`、`project_completed_run_facts`、`_record_explanation_artifact_created`、`_record_plan_confirmed`、`_record_practice_artifact_created`、`_record_grade_result_confirmed`（L25-L366） | 按事实来源而非 workflow 名分派：显式主题、讲解 Artifact、经批准计划、练习 Artifact 和真实评分分别写五类事实；Plan 复核同 Run approved 审批，所有投影都有稳定幂等语义 |
...
- 状态：进行中（2026-07-26 已落地 topic、explanation、practice、approved plan 与 Grade 投影边界）。`project_topic_confirmed_fact`（L25-L79）在 Router 前保留显式主题；`thread_memory` 继承主题不重复写。
- Artifact 完成投影由 `project_completed_run_facts`（L82-L97）、`_record_explanation_artifact_created`（L100-L138）、`_record_plan_confirmed`（L141-L206）、`_record_practice_artifact_created`（L209-L259）与 `_record_grade_result_confirmed`（L262-L366）负责。Plan 必须携带 approval ID 且数据库存在同 Run approved 审批；未批准计划不写长期目标。
