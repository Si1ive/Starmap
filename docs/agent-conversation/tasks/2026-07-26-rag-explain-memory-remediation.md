...
- Plan 事实投影的前置审批边界已修复：`backend/app/modules/agent/service.py::AgentService.decide_approval`（L424-L476）只在 approved 时恢复并投递，rejected 转为 failed、删除 checkpoint 且不产生 Artifact；`backend/app/modules/agent/workflows/plan.py::_apply_plan_change_node`（L165-L189）会从数据库复核 approval 状态，防止旁路恢复。回归见 `backend/tests/test_agent_plan_worker.py::test_rejected_plan_stops_without_outbox_or_artifact`（L89-L121）、`test_plan_apply_node_rejects_unapproved_checkpoint`（L125-L136）与 `test_approved_plan_resumes_and_creates_artifact`（L140-L166）。
