...
- `backend/app/modules/agent/memory_item_projection.py::project_trusted_memory_event`（L153-L165）只物化明确可信的两类派生：`_project_topic_context`（L60-L90）按事实键 upsert 线程主题，`_project_confirmed_plan_goal`（L108-L150）复核同 Run Plan Artifact/approval 后写用户目标；Explain/Practice/Grade 不复制权威正文。`backend/tests/test_agent_memory_outbox.py`（L110-L310）覆盖竞争、租约、topic/plan 物化、重放与失败隔离，`backend/tests/test_agent_worker_lifecycle.py::test_worker_loop_scans_run_and_memory_outboxes`（L33-L59）覆盖运行时接线。历史摘要、Embedding 与偏好候选仍待实现。
