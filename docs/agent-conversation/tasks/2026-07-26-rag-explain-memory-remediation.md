...
| Validate 检索与首个记忆消费 | `backend/app/modules/agent/memory_selector.py`、`backend/app/modules/agent/workflows/validate.py` | `_load_excluded_question_ids`、`_load_unique_weak_topic`、`_load_chapter_ids`、`load_practice_bundle`、`build_practice_query`、`build_practice_filters`、`_question_is_eligible`、`_load_learning_evidence_node`、`_question_discovery_node`、`_question_gate_node`、`_composition_gate_node`、`_render_artifact_node` | Validate 已按 `PracticeBundle` 消费 snapshot topic、aliases、difficulty、knowledge point IDs、标准章节 ID 和选中的 Artifact；`load_practice_bundle` 从近期 `practice_artifact_created` 事实事件装载真实 `excluded_question_ids`，快照无主题时回退唯一低掌握度薄弱点（恰好一个 `mastery_score < 0.6` 且有真实证据的知识点，多个则维持澄清），已解析知识点时从 `knowledge_point_chapter_links` 装载 `chapter_ids`（主章节优先）；练习产物 content 携带 `question_ids`；缺少主题时创建 `practice_topic` 等待补充后恢复 |
...
- 已冻结并落地 `chapter_ids` 稳定来源（2026-07-26）。设计口径为三级：① 用户显式章节约束（如“第三章的题”，属于 MEM-003 指代消解范围，暂未实现）；② 已解析知识点 → `knowledge_point_chapter_links` 读标准章节 ID（主章节优先、按关联度降序，本次已实现，见 `_load_chapter_ids()`）；③ 都没有时不传 `chapter_ids`，交给检索层 `search_with_outline_expansion` Phase 0 的大纲扩展自动定位章节（既有能力，用户说“二叉树”无需知道章节）。Validate 已把 bundle `chapter_ids` 下发给 `retrieve_knowledge()`。覆盖测试：selector 两个场景断言主章节排序与薄弱点链路的章节继承，workflow 断言 `chapter_ids` 透传。
...
- Plan 审批与事实闭环：`AgentService.decide_approval`（L424-L476）只在 approved 时恢复；`backend/app/modules/agent/workflows/plan.py::_apply_plan_change_node`（L165-L189）复核审批，`_render_plan_result_node`（L192-L215）把 approval ID 放入 Artifact；`backend/app/modules/agent/memory_projection.py::_record_plan_confirmed`（L185-L251）再次查询 approved 事实并按 approval ID 幂等写 `plan_confirmed`。`backend/tests/test_agent_plan_worker.py` 三条测试（L92-L200）覆盖拒绝/旁路零事实、批准写事实与重放。
- Memory Outbox 可靠生产已完成：`backend/app/modules/agent/memory_projection.py::_ensure_memory_update_outbox`（L27-L64）在事实同一事务写 pending 任务；五类新事实和已有事实重放都经过 ensure，迁移前缺任务可补建；数据库 `(run_id,event_type)` 唯一约束与 SAVEPOINT 收敛并发重复。`backend/tests/test_agent_memory_projection.py::test_topic_confirmed_projection_is_idempotent_and_skips_inherited_topic`（L133-L208）覆盖新建、重放幂等和删除后补建。实际派生 projector 与后台循环接入仍待实现。
- Memory Outbox 消费内核已完成：`backend/app/modules/agent/memory_outbox.py::MemoryOutboxStore`（L34-L181）负责到期扫描、原子认领、租约过期回收、worker 所有权、延迟重试与 failed 收敛；`MemoryOutboxConsumer`（L184-L285）在 SAVEPOINT 内校验并调用 projector，异常只回滚派生写入，不修改成功 Run。`backend/tests/test_agent_memory_outbox.py`（L105-L224）覆盖竞争、崩溃接管、重放、失败隔离和预算耗尽。当前默认 projector 仅确认可信事实，且尚未接入后台循环；实际摘要/Embedding/长期目标投影落地后再启用消费，避免提前完成任务。
