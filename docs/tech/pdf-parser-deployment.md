...
当前项目的 PDF 解析器**不是独立远程服务**，而是由后端进程在本地直接导入 Python 依赖并执行：

- `Docling` 通过 `docling.document_converter.DocumentConverter`
- `MinerU` 通过 `mineru.cli.common.convert_single_pdf`

因此，**现阶段最合适的部署方式不是单独起一个“解析器容器”给后端远程调用**，而是：

1. 继续以 `Podman` 作为统一容器运行时。
2. 将“当前激活解析器”的依赖安装进 `backend` 容器镜像。
3. 任一时刻只保留一套激活解析器依赖作为生产主路径。
4. 在管理端“系统设置 -> PDF解析器”完成配置切换。
5. 切换动作与 `backend` 容器重建 / 重启一起执行。

换句话说：

- **要用 Podman**：是，和现有项目基线一致。
- **要不要单独给解析器再开一个 Podman 容器**：当前阶段不建议。
