...
def _conversation_inputs(context: ExecutionContext):
    ...
    deps = ExplanationDeps(
        ...
        teaching_mode=teaching_policy.teaching_mode,
        need_diagnostic_check=teaching_policy.need_diagnostic_check,
        target_knowledge_point_ids=tuple(teaching_policy.target_knowledge_point_ids),
    )
    return bundle, current_input, deps, bundle.to_message_history()


async def _evidence_loop_node(
    ...
    for turn in range(max_turns):
        ...
        try:
            ...
            logger.warning(
                "Loop 预算耗尽，提前结束", run_id=context.run_id, error=str(e)
            )
            break
    ...
    logger.info(
        "证据收集完成", run_id=context.run_id, evidence_count=len(collected_evidence)
    )
...
async def _generate_explanation_node(
    context: ExecutionContext, db: AsyncSession
) -> NodeResult:
    """生成结构化讲解"""
    ...
    for entry in evidence:
        for item in entry.get("result", {}).get("results", []):
            evidence_items.append(
                {
                    "title": item.get("entity_title")
                    or item.get("title")
                    or "未命名资料",
                    "content": str(
                        item.get("content_text") or item.get("content") or ""
                    )[:800],
...
async def _render_artifact_node(
    ...
    source_section = ""
    if citations:
        source_lines = [f"- {item.get('title') or '未命名资料'}" for item in citations]
        source_section = (
            "## 本次知识库检索来源\n\n"
...
def build_explain_workflow() -> WorkflowDefinition:
    ...
    wf.add_node(
        Node(
            ...
            node_type="action",
            execute=_generate_explanation_node,
            description="生成讲解",
        )
    )
    wf.add_node(
