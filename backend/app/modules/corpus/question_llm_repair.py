...
class LLMFallbackFixer:
    ...
    def _build_fix_prompt(
        ...
        if (
            0 <= target_idx < len(context)
            and looks_like_subjective_question(context[target_idx])
        ):
            return self._build_subjective_fix_prompt(
                context,
                target_idx,
                issue,
            )
        return self._build_choice_fix_prompt(context, target_idx, issue)
