...
def test_agent_admin_routes_use_the_admin_namespace():
    ...
    expected_paths = (
        ...
        "/api/v1/admin/agent-runs/memory-outbox",
        "/api/v1/admin/agent-runs/memory-outbox/{outbox_id}",
        "/api/v1/admin/agent-runs/memory-outbox/{outbox_id}/replay",
    )
    ...
    assert "GET" in methods["/api/v1/admin/agent-runs/memory-outbox"]
...
def test_crawler_config_routes_are_owned_by_config_router():
    ...
    assert (
        routes["/api/v1/admin/crawler/config"].endpoint.__module__
        == "app.modules.crawler.config_router"
    )


def test_crawler_schedule_routes_are_owned_by_schedule_router():
    routes = _routes_by_path()
    ...
    for path in (
        ...
        assert (
            routes[path].endpoint.__module__
            == "app.modules.crawler.schedule_router"
        )
...
def test_crawler_task_routes_are_owned_by_task_router():
    ...
    for path in (
        ...
        assert (
            routes[path].endpoint.__module__
            == "app.modules.crawler.task_router"
        )
